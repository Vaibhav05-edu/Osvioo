...
class Account
{
     ...
     * @return \Illuminate\Http\Client\Response
     */
    public function getAccount(string|null $token, MediaPlatform|null $mediaPlatform): \Illuminate\Http\Client\Response
    {
    ...
    public function send(SocialPost $post): array
    {
        try {
            ...
            if ($errorCode === 'spam_risk_too_many_pending_share') {
                $errorMessage = translate('Too many pending posts. Please wait for previous videos to finish processing.');
            } elseif ($errorCode === 'spam_risk_user_banned_from_posting') {
                $errorMessage = translate('Account is temporarily banned from posting.');
            }
