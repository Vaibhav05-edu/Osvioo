...
class InstallerController extends Controller
{
     ...
     */
    public function lk(): View|RedirectResponse
    {
        if (Hash::check(base64_decode('cmVxdWlyZW1lbnRz'), request()->input('verify_token'))) {
            session()->put('system_requirments', true);

            return redirect()->route('install.db.setup', [
                'verify_token' => bcrypt(base64_decode('ZGJzZXR1cF8='))
            ]);
        }
     ...
     */
    public function envatoVerification_old(): View|RedirectResponse
    {
     ...
     * @return View|RedirectResponse
     */
    public function purchaseVerification_old(Request $request): View|RedirectResponse
    {
        $request->validate([
    ...
    {
        // ✅ force bypass
        session()->put('purchase_code', 'LOCAL');
        session()->put('username', 'LOCAL');

        // fake requirement pass
        session()->put('system_requirments', true);
