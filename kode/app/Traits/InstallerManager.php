...
trait InstallerManager
{
    ...
    /**
     * Format a value for .env file - wrap in quotes if it contains spaces or special characters
     */
    private function _formatEnvValue(?string $value): string
    {
        if ($value === null || $value === '') {
            return '';
        }

        // If the value contains spaces, quotes, or special characters, wrap it in double quotes
    ...
    private function _envConfig(Request $request): mixed
    {
        try {
            ...
            // Get values that may contain spaces
            ...
            $output =
                'APP_NAME=' . $this->_formatEnvValue($appName) . PHP_EOL .
                ...
                'DB_PASSWORD=' . $this->_formatEnvValue($dbPassword) . PHP_EOL .
                ...
                'PURCHASE_KEY=' . $this->_formatEnvValue($purchaseKey) . PHP_EOL .
                'ENVATO_USERNAME=' . $this->_formatEnvValue($envatoUsername) . PHP_EOL;
